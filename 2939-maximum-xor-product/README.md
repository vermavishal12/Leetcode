<h2><a href="https://leetcode.com/problems/maximum-xor-product">2939. Maximum Xor Product</a></h2><h3>Medium</h3><hr>

<p>Given three integers <code>a</code>, <code>b</code>, and <code>n</code>, return the <strong>maximum value</strong> of <code>(a XOR x) * (b XOR x)</code> where <code>0 <= x < 2<sup>n</sup></code>.</p>
...
It can be shown that 98 is the maximum value of (a XOR x) * (b XOR x) for all 0 <= x < 2<sup>n</sup>.
</pre>
...
It can be shown that 930 is the maximum value of (a XOR x) * (b XOR x) for all 0 <= x < 2<sup>n</sup>.
</pre>
