...
/* _*_ SOLUTION*/
// so from the previous solution we just updated these to 
// sol1 is the product of the ((a xor x) * (b xor x)) when the bit is unset and sol2 is the product of the ((a xor x) * (b xor x)) when the bit is set
// instead we check that if we set the ith bit it doesn't make the ith bit of (min(a,b) xor x) to zero
// this will return the maximized result
    ...
    int maximumXorProduct(long long a, long long b, int n) {
        ...
        for(ll i = n-1; i >= 0; i--) {
            ...
            if(binaryValA[i] == binaryValB[i]){
                ...
                if ((min(a,b) & val) == 0){     // updated the last code
                    a = a^val;
                    b = b^val;
                }
            }
